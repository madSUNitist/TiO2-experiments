...
/ 模拟参数: 采用 pymatgen ``XRDCalculator`` ($"Cu" "K"alpha$, 波长 $1.5406 angstrom$, symprec = 0.1) , $2theta$ 扫描范围 $10 degree - 80 degree$, 与实验条件一致。模拟过程自动处理对称性等价反射 (multiplicity) 、原子散射因子和 Lorentz-极化因子。最终峰表不经任何事后修改 (如 JCPDS 数据库查询) 。
...
综合 Spurr-Myers (100% Anatase) 、NNLS (以 Anatase 为主, 但信度有限) 以及独立峰共识 (Anatase CONFIRMED, Rutile TENTATIVE) 三方面判断, 样品为*纯锐钛矿相*。后续讨论中仅以锐钛矿的晶体学参数进行分析。
...
由于金红石相未通过独立峰共识检验, 仅对锐钛矿特征峰进行 Scherrer 分析。锐钛矿 (101) 面 ($2theta = 25.29 degree$) 的 FWHM 为 $1.1339 degree$, 经仪器展宽校正后计算得晶粒尺寸 $D = 7.1$ nm。其余锐钛矿衍射峰因与相邻峰间距过小 ($< 1.5 degree$) , 无法可靠分离半高宽, 未纳入定量分析。
...
经三层过滤的严谨验证, 样品最终鉴定为*纯锐钛矿相* (Anatase), 金红石相未能通过独立主峰共识检验 (2/3, 低于 3 峰门槛) 。Spurr-Myers 公式在噪声过滤后直接退化为 100% Anatase。锐钛矿 (101) 面衍射峰 ($2theta approx 25.3 degree$) 为最强峰, 与锐钛矿纳米材料的典型衍射特征一致。
...
Scherrer 公式估算的锐钛矿晶粒尺寸约 7 nm, 远小于 SEM 观测的颗粒粒径 (中位 630 nm) 。这一差异符合纳米材料 XRD-SEM 联合表征的典型特征：SEM 图像中的颗粒为多晶聚集体, 单个颗粒包含多个晶粒 (相干衍射域) 。较小的锐钛矿晶粒 (约 7 nm) 意味着较大的比表面积, 有利于光催化反应中活性位点的暴露。
...
+ XRD 分析表明样品为*纯锐钛矿相* (Anatase), 金红石相未通过独立峰共识检验 (2/3) , Spurr-Myers 公式经噪声过滤后指示 100% Anatase; Scherrer 公式估算晶粒尺寸约 7 nm, 与 SEM 粒径差异表明颗粒为多晶聚集体。高速扫描 ($10 degree"/min"$) 限制了 XRD 精细分析的能力
