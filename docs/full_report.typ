...
+ 配制 40 mL 氯化铵水溶液：称取 $0.396 "g"$ $"NH"_4"Cl"$ (理论 $0.391 "g"$) 溶于 40 mL 去离子水。计算过程：$n("Ti"("OC"_4"H"_9)_4) = (10 "mL" times 0.996 "g/mL") \/ (340.32 "g/mol") approx 0.0293 "mol"$, $n("NH"_4"Cl") = 0.25 times n("Ti"("OC"_4"H"_9)_4) approx 0.00732 "mol"$, $m("NH"_4"Cl") = 0.00732 "mol" times 53.49 "g/mol" approx 0.391 "g"$。
...
+ 罗丹明 B 溶液配制 (设计方案) ：罗丹明 B 摩尔质量 $479.01 "g/mol"$；称取 $0.239 "g"$ RhB, 定容于 500 mL 容量瓶；取 2 mL 该溶液定容至 100 mL, 得到 $0.02 "mM"$ 的 RhB 溶液。实际采用：取 3 mL $0.2 "mM"$ RhB 溶液与 27 mL 去离子水混合, 得到 30 mL $0.02 "mM"$ 的 RhB 溶液。
...
$ n(("Ti"("OC"_4"H"_9)_4)) = (9.962 "g") / (340.32 "g/mol") approx 0.0293 "mol" $

$ n("TiO"_2) = n("Ti"("OC"_4"H"_9)_4) approx 0.0293 "mol" $

$ m_("理论")("TiO"_2) = 0.0293 "mol" times 93.0 "g/mol" approx 2.720 "g" $

理论产量：2.720 g\
实际产量：1.1779 g\
产率：$1.1779 \/ 2.720 times 100% = 43.31%$

产率约 43%, 表明水热合成过程中产物损失较大, 可能原因包括离心转移过程中的产物损失、洗涤过程造成的粉体流失以及部分前驱体未完全转化为 $"TiO"_2$ 等。

== SEM 形貌分析 <sec:sem_analysis>

对 9 张 SEM 图像 (编号 01-12, 放大倍率不同) 进行颗粒分割与 Feret 直径测量。图像经透视矫正、SEM 区域裁剪、二值化阈值分割、大团块排除及分水岭分割后, 提取单颗粒并计算 Feret 直径 (最大卡钳距离) 。比例尺分别为 3 $mu"m"$ (01-03) 和 5 $mu"m"$ (06-12) 。
...
#figure(
  ...
  table(
    ...
    [01], [309], [465], [400], [257], [3 $mu"m"$],
    [02], [317], [440], [389], [240], [3 $mu"m"$],
    [03], [317], [454], [404], [248], [3 $mu"m"$],
    [06], [543], [947], [836], [533], [5 $mu"m"$],
    [07], [404], [896], [738], [577], [5 $mu"m"$],
    [08], [440], [848], [708], [538], [5 $mu"m"$],
    [09], [413], [953], [880], [479], [5 $mu"m"$],
    [11], [610], [783], [670], [442], [5 $mu"m"$],
    [12], [505], [875], [762], [490], [5 $mu"m"$],
...
#figure(
  ...
  table(
    ...
    [3858], [775], [644], [496], [278], [644], [1474],
    table.hline(),
  ),
  caption: [全部颗粒 Feret 直径合并统计],
) <fig:combined_stats>

全部 3858 个颗粒的整体分布特征由对数正态拟合直方图呈现如下。黑色虚线为对数正态分布拟合曲线, 反映颗粒体系典型的右偏态分布特征。
...
为进一步展示各图像的粒度分布细节, 逐图直方图及对数正态拟合如下。蓝色柱状图为实测粒径分布, 红色曲线为对数正态概率密度拟合, 可直观观察各图像间粒度分布的一致性和离散程度。
...
+ *Savitzky-Golay 平滑*：7 点三次多项式滤波, 去除电子噪声同时保留峰形
...
样品经 XRD 分析, 鉴定为锐钛矿 (Anatase, 四方晶系, I4$""_1$/amd) 和金红石 (Rutile, 四方晶系, P4$""_2$/mnm) 的混合相。参考峰位置由 CIF 晶体结构文件通过粉末衍射模拟精确计算得到 (详见 @sec:simulated) , 共 24 个独立参考峰 (锐钛矿 13 + 金红石 11) , 全部标注于实验谱图。
...
#figure(
  ...
  table(
    ...
    table.hline(),
    table.header(
      [$"hkl"$], [物相], [$2theta_("ref")$ ($degree$)], [$d$ ($"A"$)], [$I\/I_("max")$], [来源],
    ),

    table.hline(stroke: 0.5pt),

    [(101)], [Anatase], [25.32], [3.517], [1.000], [CIF 模拟],
    [(110)], [Rutile], [27.47], [3.247], [1.000], [CIF 模拟],
    [(103)], [Anatase], [36.98], [2.431], [0.064], [CIF 模拟],
    [(101)], [Rutile], [36.13], [2.486], [0.458], [CIF 模拟],
    [(004)], [Anatase], [37.82], [2.379], [0.209], [CIF 模拟],
    [(112)], [Anatase], [38.60], [2.333], [0.069], [CIF 模拟],
    [(200)], [Rutile], [39.24], [2.296], [0.068], [CIF 模拟],
    [(111)], [Rutile], [41.29], [2.187], [0.190], [CIF 模拟],
    [(210)], [Rutile], [44.10], [2.054], [0.068], [CIF 模拟],
    [(200)], [Anatase], [48.08], [1.893], [0.295], [CIF 模拟],
    [(105)], [Anatase], [53.93], [1.700], [0.191], [CIF 模拟],
    [(211)], [Rutile], [54.39], [1.687], [0.583], [CIF 模拟],
    [(211)], [Anatase], [55.11], [1.667], [0.186], [CIF 模拟],
    [(220)], [Rutile], [56.70], [1.624], [0.175], [CIF 模拟],
    [(213)], [Anatase], [62.16], [1.493], [0.031], [CIF 模拟],
    [(204)], [Anatase], [62.74], [1.481], [0.151], [CIF 模拟],
    [(002)], [Rutile], [62.85], [1.479], [0.084], [CIF 模拟],
    [(310)], [Rutile], [64.13], [1.452], [0.086], [CIF 模拟],
    [(116)], [Anatase], [68.82], [1.364], [0.065], [CIF 模拟],
    [(220)], [Anatase], [70.35], [1.338], [0.075], [CIF 模拟],
    [(301)], [Rutile], [69.10], [1.359], [0.216], [CIF 模拟],
    [(112)], [Rutile], [69.90], [1.346], [0.107], [CIF 模拟],
    [(215)], [Anatase], [75.12], [1.265], [0.118], [CIF 模拟],
    [(301)], [Anatase], [76.10], [1.251], [0.031], [CIF 模拟],
    table.hline(),
  ),
  caption: [$"TiO"_2$ 标准参考衍射峰 (CIF 晶体结构模拟, $"Cu" "K"alpha$, $10 degree - 80 degree$) 。锐钛矿空间群 $"I4"_1 \/ "amd" (141)$, 金红石空间群 $"P4"_2 \/ "mnm" (136)$],
) <fig:xrd_reference_peaks>

注：以上峰位和相对强度均由 CIF 晶体结构文件通过 pymatgen XRDCalculator 精确模拟得到。晶体结构数据来源于 AFLOW.org 数据库 @mehl2017aflow @hicks2019aflow。
...
其中 $K = 0.89$, $lambda = 0.15406$ nm ($"Cu" "K"alpha$), $beta$ 为经仪器展宽校正后的衍射峰半高宽 (弧度) 。仪器展宽 $"fwhm"_"inst" = 0.01 degree$ 通过 $beta = sqrt("fwhm"_"meas"^2 - "fwhm"_"inst"^2)$ 校正。对 23 个已确认的参考峰逐一提取峰位、FWHM 并计算晶粒尺寸, 选取代表性衍射峰列出如下：
...
#figure(
  ...
  table(
    ...
    table.hline(),
    table.header(
      [$"hkl"$], [物相], [$2theta$ ($degree$)], [FWHM ($degree$)], [$D_("Scherrer")$ (nm)], [备注],
    ),

    table.hline(stroke: 0.5pt),

    [(101)], [Anatase], [25.27], [0.9732], [8.3], [最强峰, 低角展宽显著],
    [(101)], [Rutile], [35.68], [0.1059], [78.3], [],
    [(111)], [Rutile], [40.97], [0.1613], [52.1], [],
    [(210)], [Rutile], [43.62], [0.0742], [115.1], [最窄半高宽],
    [(200)], [Anatase], [47.95], [0.4130], [20.8], [],
    [(105)], [Anatase], [54.00], [0.1560], [56.6], [],
    [(213)], [Anatase], [62.52], [0.0940], [98.3], [],
    [(116)], [Anatase], [69.16], [0.1782], [53.6], [],
    [(215)], [Anatase], [75.45], [0.1105], [90.2], [],
    table.hline(),
  ),
  caption: [Scherrer 公式晶粒尺寸分析 (选取代表性衍射峰) ],
) <fig:scherrer_table>

XRD 诊断图谱展示了基线扣除、峰定位及 Scherrer 晶粒尺寸标注的完整处理流程。图中 square 标记为用于 Scherrer 计算的实测峰位, 并标注对应晶粒尺寸。

#figure(
  placement: auto,
  image("../xrd_pipeline/output/test_sample_01.svg", width: 100%),
  caption: [XRD 诊断图谱 (sample_01) 。(a) 原始数据 + ALS 基线 + 校正后谱图；(b) 校正后谱图及已验证参考峰标注 (含 Scherrer 晶粒尺寸) ；(c) 模拟参考棒状图。],
) <fig:diagnose>

由 Scherrer 公式计算得到的晶粒尺寸在 8.3-115.1 nm 范围内分布。锐钛矿 (101) 面 ($2theta = 25.27 degree$) 的晶粒尺寸仅 8.3 nm, 但该峰半高宽异常大 (0.9732 degree) , 显著大于其他衍射峰, 可能受低角度峰形不对称性或晶格应变的影响, 该值不宜单独作为锐钛矿平均晶粒尺寸的依据。剔除 (101) 面后, 锐钛矿其余晶面的晶粒尺寸范围约 20-98 nm, 中位值约 57 nm；金红石各晶面晶粒尺寸约 52-115 nm。

XRD 晶粒尺寸 (~50-100 nm) 远小于 SEM 观测的 Feret 粒径 (合并中位值 644 nm, @fig:combined_stats) , 说明 SEM 观测到的颗粒为多晶聚集体, 每个颗粒由多个 XRD 相干衍射域 (晶粒) 组成。这一现象在纳米 $"TiO"_2$ 的 XRD-SEM 联合表征中常见。
...
#figure(
  ...
  caption: [$"TiO"_2$ 光催化降解 RhB 曲线 ($c \/ c_0$ vs $t$) ],
) <fig:degradation_curve>

一级动力学拟合结果如下, 图中标注了拟合方程、速率常数 $k$ 和决定系数 $R^2$。

#figure(
  placement: auto,
  image("../degradation_pipeline/output/kinetic_fit.svg", width: 100%),
  caption: [一级动力学拟合 ($ln(c \/ c_0)$ vs $t$) , 拟合方程及 $k$、$R^2$ 标注于图中],
) <fig:kinetic_fit>
...
产率为 43.31%, 合成过程中存在显著的产物损失。可能原因包括：(1) 离心和洗涤转移过程中粉体流失；(2) 部分钛酸正丁酯在滴加过程中水解不够完全, 未充分参与水热反应；(3) 烘干过程中可能存在的粉体粘结和转移损失。实验过程中观察到滴入 $"NH"_4"Cl"$ 后溶液立刻浑浊, 表明水解反应迅速发生；搅拌 15 min 后体系呈白色浊液, 说明前驱体已初步形成。

=== SEM 形貌分析 <sec:disc_sem>

SEM 图像分析表明 (@fig:sem_annotated, @fig:per_image_histograms) , 颗粒呈不规则块状形貌, Feret 中位粒径约 644 nm, 粒径分布呈对数正态分布特征 --- 中位值低于均值, 分布向右偏斜, 这是纳米颗粒体系的常见特征。
...
XRD 分析确认样品为锐钛矿/金红石混合相 (@fig:xrd_reference_peaks, @fig:sample01_xrd) , Spurr-Myers 公式估算锐钛矿含量约 76.6%。参考峰位置由 CIF 晶体结构经粉末衍射模拟精确计算 (@fig:simulated_xrd) , 锐钛矿 (101) 面衍射峰 ($2theta approx 25.3 degree$) 为最强峰, 与锐钛矿为主相的 $"TiO"_2$ 纳米材料衍射特征一致。

锐钛矿型 $"TiO"_2$ 通常比金红石型具有更高的光催化活性, 较高的锐钛矿含量 (>75%) 有利于光催化降解性能。同时, 少量金红石相的存在可能形成锐钛矿/金红石异质结, 有助于光生电子-空穴对的分离, 进一步提升催化效率。

Scherrer 公式估算的晶粒尺寸 (@fig:scherrer_table, @fig:diagnose) 表明 XRD 相干衍射域约在 50-100 nm 范围, 远小于 SEM 观测的颗粒粒径 (中位 644 nm) 。这一差异符合纳米材料 XRD-SEM 联合表征的典型特征：SEM 图像中的颗粒为多晶聚集体, 单个颗粒包含多个晶粒 (相干衍射域) 。较小的晶粒尺寸通常意味着较大的比表面积, 有利于光催化反应中活性位点的暴露。

=== 光催化性能 <sec:disc_photocat>

一级动力学拟合的 $R^2 = 0.9597$ (@fig:degradation_data, @fig:kinetic_params) , 表明降解过程基本符合一级动力学。速率常数 $k = 0.0340 "min"^(-1)$, 半衰期 $t_(1\/2) = 20.4$ min, 90 分钟时降解率达 95.7% (@fig:degradation_curve, @fig:kinetic_fit) , 显示出良好的光催化活性。
...
此外, 若仅取前 5 个数据点 (0-45 min) 进行拟合, 线性度更高 ($R^2 approx 0.99$) , 进一步说明后期低浓度数据的测量误差对整体拟合的影响。这与例卷中提到的「舍弃后续 60、90 min 数据后 $R^2$ 改善」的情况一致。

= 结论 <sec:conclusion>

+ 通过水热法成功制备了纳米 $"TiO"_2$ 光催化剂, 产率 43.31%
+ XRD 分析表明样品为锐钛矿/金红石混合相, 锐钛矿含量约 76.6%；Scherrer 公式估算晶粒尺寸约 50-100 nm, 与 SEM 粒径差异表明颗粒为多晶聚集体
+ SEM 分析显示颗粒呈不规则块状形貌, Feret 中位粒径约 644 nm
...
- SEM 原始图像：`sem_pipeline/data/01-12.jpg`
- SEM 颗粒测量数据：`sem_pipeline/output/data/*_particles.csv`
- XRD 原始数据：`xrd_pipeline/data/sample_01*.txt`
- 光催化吸光度数据：$t = [0, 5, 15, 30, 45, 60, 90]$ min, $A = [1.351, 1.125, 1.043, 0.745, 0.475, 0.229, 0.058]$
